...
public sealed class ConsumerDecisionTests
{
    ...
    {
        private static readonly Headers EmptyHeaders = TestHeaders.Create();
    ...
    private class TestHeaders : DispatchProxy
    {
        public static Headers Create() =>
            DispatchProxy.Create<Headers, TestHeaders>();

        protected override object? Invoke(MethodInfo? targetMethod, object?[]? args)
        {
            if (targetMethod?.Name == "TryGetHeader")
            {
                args![1] = null;
                return false;
            }
            ...
            throw new NotSupportedException($"Unexpected Headers member: {targetMethod?.Name}");
        }
    }
}
