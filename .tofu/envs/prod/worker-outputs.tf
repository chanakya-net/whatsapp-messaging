...
output "smoke_job_outbound_ip_addresses" {
  description = "Prod smoke job egress addresses for downstream database firewall reconciliation."
...
output "reviewed_postgres_egress" {
  description = "Complete reviewed prod PostgreSQL egress, labelled by source and deduplicated."
  value = {
    environment = "prod"
