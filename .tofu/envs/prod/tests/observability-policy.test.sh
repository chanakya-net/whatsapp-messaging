...
ENVIRONMENT_FILES=("$REPO_ROOT/.tofu/envs/prod"/*.tf)
...
if rg -n 'resource[[:space:]]+"azurerm_(log_analytics_workspace|application_insights|monitor_workspace|monitor_data_collection_(endpoint|rule)|dashboard_grafana|prometheus_rule_group)"' "${ENVIRONMENT_FILES[@]}"; then
  printf '%s\n' 'FAIL: Prod must not provision Log Analytics, Application Insights, or Prometheus-managed resources.' >&2
  exit 1
fi
...
printf '%s\n' 'Prod direct New Relic telemetry policy checks passed.'
