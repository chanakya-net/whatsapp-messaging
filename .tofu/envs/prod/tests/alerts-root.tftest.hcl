mock_provider "azurerm" {
  ...
  mock_resource "azurerm_container_app" {
    defaults = {
      ...
      outbound_ip_addresses = ["20.193.0.20"]
    }
  }

  mock_resource "azurerm_container_app_job" {
    defaults = {
      id                    = "/subscriptions/00000000-0000-4000-8000-000000000002/resourceGroups/rg-messagebridge-prod-centralindia-042/providers/Microsoft.App/jobs/job-messagebridge-prod-cin-042"
      outbound_ip_addresses = ["20.193.0.30"]
    }
  }
}
...
run "wires_one_email_action_group_and_environment_alerts" {
  ...
  assert {
    condition = (
      azurerm_monitor_action_group.alerts.name == "ag-messagebridge-prod-cin-042" &&
    ...
    error_message = "Prod must own exactly one email Action Group."
  }
  ...
  assert {
    ...
    error_message = "Prod must wire three worker alerts and one failure alert per migration/smoke job."
  }
  ...
  assert {
    ...
    error_message = "Prod job alerts must retain job ownership and reviewed severities."
  }
}
