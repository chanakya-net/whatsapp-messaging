...
ENVIRONMENT_DIR="$REPO_ROOT/.tofu/envs/shared"
...
if grep -En "$FORBIDDEN_RESOURCES" "${SOURCES[@]}"; then
  printf '%s\n' 'FAIL: Shared alerts must not provision log analytics, application insights, query alerts, processing rules, or managed Prometheus.' >&2
  exit 1
fi
...
if [[ "$ACTION_GROUP_COUNT" != "1" ]]; then
  printf 'FAIL: Shared must define one Action Group; found %s declarations.\n' "$ACTION_GROUP_COUNT" >&2
  exit 1
fi

printf '%s\n' 'Shared Azure-native alert cost-policy checks passed.'
