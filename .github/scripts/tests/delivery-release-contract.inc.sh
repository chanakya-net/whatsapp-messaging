...
assert_happy_release_calls() {
  local call_log=$1 environment=$2 migration_update migration_start capture worker_update revision smoke_start
  ...
  ((migration_update < migration_start && migration_start < capture && capture < worker_update && \
    worker_update < revision && revision < smoke_start)) || fail "$environment release ordering is unsafe."
...
assert_release_calls() {
  local case_json=$1 call_log=$2 environment=$3 name expected
  name="$(jq -r '.name' <<<"$case_json")"
  [[ "$(grep -c '^containerapp job update' "$call_log")" == 1 ]] || fail "$name must update migration image once."
  if [[ "$name" == migration-update-failure ]]; then
    [[ "$(grep -c 'containerapp job start.*mig-messagebridge' "$call_log" || true)" == 0 ]] ||
      fail "$name must stop before migration start."
  else
  ...
  if [[ "$name" == release-success ]]; then
    assert_happy_release_calls "$call_log" "$environment"
...
run_release_case() {
  local release_script=$1 test_dir=$2 case_json=$3 environment=$4 name output_file call_log state_dir exit_code=0
  name="$(jq -r '.name' <<<"$case_json")"
  ...
  PATH="$test_dir/bin:$PATH" FAKE_AZ_CASE="$case_json" FAKE_AZ_CALL_LOG="$call_log" \
    ...
    MIGRATION_JOB_NAME="mig-messagebridge-$environment-cin-042" SMOKE_JOB_NAME="smoke-messagebridge-$environment-cin-042" \
    WORKER_NAME="ca-messagebridge-$environment-cin-042" RESOURCE_GROUP="rg-messagebridge-$environment-centralindia-042" \
  ...
  assert_release_calls "$case_json" "$call_log" "$environment"
}
...
test_dev_release_scenarios() {
  ...
  while IFS= read -r case_json; do
    run_release_case "$release_script" "$test_dir" "$case_json" dev
  done < <(jq -c '.release[]' "$FIXTURES")
}

test_prod_release_scenarios() {
  local release_script fixture_az test_dir case_json
  release_script="$(step_script 'Migrate, deploy, smoke, and roll back production')"
  [ -n "$release_script" ] || fail 'Production release script is missing.'
  fixture_az="$REPO_ROOT/.github/scripts/tests/fixtures/delivery/dev-release/fake-az.sh"
  [ -f "$fixture_az" ] || fail 'Reusable release Azure fixture is missing.'
  test_dir="$(mktemp -d)"
  trap 'rm -rf -- "$test_dir"' RETURN
  mkdir -p "$test_dir/bin"
  install -m 700 "$fixture_az" "$test_dir/bin/az"
  while IFS= read -r case_json; do
    run_release_case "$release_script" "$test_dir" "$case_json" prod
  done < <(jq -c '.release[]' "$FIXTURES")
}
